export default class npyjs {
	...
	parse(arrayBufferContents) {
		// npz version
		const headerLengthNpz = new DataView(
			arrayBufferContents.slice(67, 69)
		).getUint8(0);
		const offsetBytesNpz = 69 + headerLengthNpz;

		console.log(headerLengthNpz);

		let hcontents2 = new TextDecoder("utf-8").decode(
			new Uint8Array(arrayBufferContents.slice(0, 1000))
		);
		console.log(hcontents2);

		let hcontents = new TextDecoder("utf-8").decode(
			new Uint8Array(arrayBufferContents.slice(69, 69 + headerLengthNpz))
		);
		...
		let nums = new Uint8Array(arrayBufferContents, offsetBytesNpz);
